    ...
    function movePoints(time) {
      ...
      lines.forEach((pts) => {
        pts.forEach((p) => {
          const move =
            noise.perlin2(
              (p.x + time * speedX) * 0.002,
              (p.y + time * speedY) * 0.0015
            ) * 12;
          ...
          if (dist < l) {
            ...
            p.cursor.vx += Math.cos(mouse.a) * force * l * mouse.vs * 0.00065;
            p.cursor.vy += Math.sin(mouse.a) * force * l * mouse.vs * 0.00065;
          }

          p.cursor.vx += (0 - p.cursor.x) * t;
          p.cursor.vy += (0 - p.cursor.y) * t;
          p.cursor.vx *= f;
          p.cursor.vy *= f;
